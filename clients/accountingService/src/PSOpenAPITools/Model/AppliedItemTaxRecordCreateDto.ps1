...
No description available.
.PARAMETER BillingItemRecordId
No description available.
.OUTPUTS

AppliedItemTaxRecordCreateDto<PSCustomObject>
#>

function Initialize-AppliedItemTaxRecordCreateDto {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => AppliedItemTaxRecordCreateDto' | Write-Debug
        ...
        if (!$BillingItemRecordId -and $BillingItemRecordId.length -gt 36) {
            throw "invalid value for 'BillingItemRecordId', the character length must be smaller than or equal to 36."
        }

        if (!$BillingItemRecordId -and $BillingItemRecordId.length -lt 0) {
            throw "invalid value for 'BillingItemRecordId', the character length must be great than or equal to 0."
        }
        ...
        $PSO = [PSCustomObject]@{
            ...
            "billingItemRecordId" = ${BillingItemRecordId}
        }
...
Convert from JSON to AppliedItemTaxRecordCreateDto<PSCustomObject>
...
AppliedItemTaxRecordCreateDto<PSCustomObject>
#>
function ConvertFrom-JsonToAppliedItemTaxRecordCreateDto {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => AppliedItemTaxRecordCreateDto' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in AppliedItemTaxRecordCreateDto
        $AllProperties = ("id", "timestamp", "taxPolicyId", "invoiceId", "itemId", "taxAmountInUSD", "taxBaseAmountInUSD", "billingItemRecordId")
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match "billingItemRecordId"))) { #optional property not found
            $BillingItemRecordId = $null
        } else {
            $BillingItemRecordId = $JsonParameters.PSobject.Properties["billingItemRecordId"].value
        }
