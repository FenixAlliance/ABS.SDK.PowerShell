...
BankGuaranteeCreateDto<PSCustomObject>
#>

function Initialize-BankGuaranteeCreateDto {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => BankGuaranteeCreateDto' | Write-Debug
        ...
        if (!$ContactId -and $ContactId.length -lt 0) {
            throw "invalid value for 'ContactId', the character length must be great than or equal to 0."
        }

        if (!$ProjectId -and $ProjectId.length -gt 36) {
            throw "invalid value for 'ProjectId', the character length must be smaller than or equal to 36."
        }

        if (!$ProjectId -and $ProjectId.length -lt 0) {
            throw "invalid value for 'ProjectId', the character length must be great than or equal to 0."
        }
...
Convert from JSON to BankGuaranteeCreateDto<PSCustomObject>
...
BankGuaranteeCreateDto<PSCustomObject>
#>
function ConvertFrom-JsonToBankGuaranteeCreateDto {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => BankGuaranteeCreateDto' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in BankGuaranteeCreateDto
